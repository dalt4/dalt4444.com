const porfolio__navLinks = document.querySelectorAll('.porfolio__nav-link'),
...
const siteLinksArray = [
    'http://xn--80audjku.xn--p1acf/',
    'https://avgm.kz/',
    'https://odigo.sitetest.pro/',
    'https://harley.sitetest.pro/',
    'https://www.drumart.ee/'
];

const descriptionSiteArray = [
    'Полный цикл разработки от дизайна до размещения на хостинге',
    'Полный цикл разработки от дизайна до размещения на хостинге',
    'Верстка макета главной страницы из Figma + адаптив',
    'Верстка главной страницы из PSD макета',
...
porfolio__navLinks.forEach((item, i) => {
    item.onclick = () => {
        porfolio__navLinks.forEach((item) => item.classList.remove('active'));
        ...
        portfolio__items.forEach((item) => item.classList.remove('active'));
        portfolio__items[i].classList.add('active');
        infoDescription.textContent = descriptionSiteArray[i];
        setTimeout(() => portfolio__items[i].style.overflowY = 'auto', 900)
    }
})
...
portfolio__closeItem.onclick = () => {
    porfolio__navLinks.forEach((item) => item.classList.remove('active'));
    portfolio__items.forEach((item) => {
